...
def terraform_apply_refresh(cwd, AUTO_APPROVE=False):
    tfvars_settings(cwd)
    if AUTO_APPROVE:
        process = subprocess.Popen(Terraform_commands_constants.APPLY_REFRESH_AUTO_APPROVE_PROCESS, cwd=cwd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=Terraform_commands_constants.ENV).wait()
    else:
        process = subprocess.Popen(Terraform_commands_constants.APPLY_REFRESH_PROCESS, cwd=cwd, env=Terraform_commands_constants.ENV).wait()
...
def terraform_apply_auto_approve_refresh(cwd):
    ...
    subprocess.Popen(Terraform_commands_constants.APPLY_REFRESH_AUTO_APPROVE_PROCESS, cwd=cwd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=Terraform_commands_constants.ENV).wait()
    ...
    #         terraform_apply_refresh(cwd)
            # subprocess.Popen(APPLY_REFRESH_PROCESS, cwd=cwd, env=Terraform_commands_constants.ENV).wait()
